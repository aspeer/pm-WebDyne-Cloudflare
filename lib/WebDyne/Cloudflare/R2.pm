package WebDyne::Cloudflare::R2;

use 5.020;
use strict;
use warnings;

use Encode qw(decode FB_CROAK);
use Future::AsyncAwait;
use WebDyne::Cloudflare ();
use JSON::PP ();
use MIME::Base64 qw(decode_base64);
use Scalar::Util qw(blessed);
use WebDyne::Cloudflare::R2::Blob;
use WebDyne::Cloudflare::R2::Error;
use WebDyne::Cloudflare::R2::Object;

our $VERSION='0.001';
our $HOST_CALL;

use constant EXTENSION_NAME => 'webdyne.cloudflare.r2';
use constant PROTOCOL_VERSION => 1;

my $json_or=JSON::PP->new()->canonical()->allow_nonref();

sub new {
    my ($class, %opt)=@_;
    my $scope_hr=$opt{'scope'};
    die "WebDyne::Cloudflare::R2 requires a PAGI scope hash\n"
        unless ref($scope_hr) eq 'HASH';
    my $extension_hr=(ref($scope_hr->{'extensions'}) eq 'HASH')
        ? $scope_hr->{'extensions'}{EXTENSION_NAME()}
        : undef;
    die "PAGI scope has no " . EXTENSION_NAME() . " capability\n"
        unless ref($extension_hr) eq 'HASH';
    die "Unsupported R2 capability protocol\n"
        unless (defined($extension_hr->{'version'})
            &&!ref($extension_hr->{'version'})
            &&($extension_hr->{'version'} eq PROTOCOL_VERSION()));
    die "Invalid R2 capability token\n"
        unless (defined($extension_hr->{'capability'})
            &&!ref($extension_hr->{'capability'})
            &&length($extension_hr->{'capability'}));

    my $binding=(defined($opt{'binding'}) ? $opt{'binding'} : 'R2');
    die "Invalid R2 binding name '$binding'\n"
        unless (!ref($binding)&&($binding=~/\A[A-Z_][A-Z0-9_]*\z/));
    my $bindings_ar=$extension_hr->{'bindings'};
    die "Invalid R2 capability binding list\n"
        unless (ref($bindings_ar) eq 'ARRAY');
    my %binding=map { $_ => 1 } grep { defined($_)&&!ref($_) } @{$bindings_ar};
    die "R2 binding '$binding' is not available to this request\n"
        unless $binding{$binding};

    my $self=bless({
        binding    => $binding,
        capability => $extension_hr->{'capability'},
    }, $class);
    return $self;
}


sub binding { return shift()->{'binding'}; }


sub blob {
    #  Accept function, class and object calls without discarding payload bytes.
    #
    shift() if ((@_>1)&&(defined($_[0])
        &&(blessed($_[0])||(!ref($_[0])&&($_[0] eq __PACKAGE__)))));
    die "R2 blob requires exactly one byte string\n" unless (@_==1);
    return WebDyne::Cloudflare::R2::Blob->new($_[0]);
}


sub key {
    my ($key)=@_;
    die "R2 key must be a non-empty scalar\n"
        unless (defined($key)&&!ref($key)&&length($key));
    return $key;
}


sub known_options {
    my ($operation, $allowed_ar, %opt)=@_;
    my %allowed=map { $_ => 1 } @{$allowed_ar};
    my @unknown=sort grep { !$allowed{$_} } keys %opt;
    die "Unknown R2 $operation option: $unknown[0]\n" if @unknown;
    return %opt;
}


sub encode_text {
    my ($value_ref)=@_;
    die "R2 object must be a scalar or R2 blob\n" if (!defined($value_ref)||ref($value_ref));
    return decode('UTF-8', $value_ref, FB_CROAK)
        if (!utf8::is_utf8($value_ref)&&($value_ref=~/[\x80-\xff]/));
    return "$value_ref";
}


sub object {
    my ($value_hr)=@_;
    return undef unless defined($value_hr);
    die "R2 host returned invalid object metadata\n" unless ref($value_hr) eq 'HASH';
    if (exists($value_hr->{'body'})) {
        my $body_hr=$value_hr->{'body'};
        die "R2 host returned an invalid object body\n"
            unless ((ref($body_hr) eq 'HASH')
                &&defined($body_hr->{'type'})&&($body_hr->{'type'} eq 'bytes')
                &&defined($body_hr->{'base64'})&&!ref($body_hr->{'base64'}));
        $value_hr->{'body'}=decode_base64($body_hr->{'base64'});
    }
    return WebDyne::Cloudflare::R2::Object->new($value_hr);
}


async sub get {
    my ($self, $key, %opt)=@_;
    %opt=known_options('get', [qw(range)], %opt);
    return object(await $self->execute(
        operation => 'get',
        key       => key($key),
        (exists($opt{'range'}) ? (range => $opt{'range'}) : ()),
    ));
}


async sub head {
    my ($self, $key)=@_;
    return object(await $self->execute(operation => 'head', key => key($key)));
}


async sub put {
    my ($self, $key, $value_ref, %opt)=@_;
    %opt=known_options('put', [qw(http_metadata custom_metadata storage_class)], %opt);
    my $encoded_ref=(blessed($value_ref)&&$value_ref->isa('WebDyne::Cloudflare::R2::Blob'))
        ? $value_ref->wire_value()
        : encode_text($value_ref);
    return object(await $self->execute(
        operation => 'put',
        key       => key($key),
        value     => $encoded_ref,
        %opt,
    ));
}


async sub delete {
    my ($self, $key)=@_;
    return await $self->execute(operation => 'delete', key => key($key));
}


async sub delete_many {
    my ($self, @keys)=@_;
    die "R2 delete_many requires at least one key\n" unless @keys;
    die "R2 delete_many accepts at most 1000 keys\n" if @keys > 1000;
    return await $self->execute(operation => 'delete', keys => [map { key($_) } @keys]);
}


async sub list {
    my ($self, %opt)=@_;
    %opt=known_options('list', [qw(prefix cursor delimiter limit include)], %opt);
    my $result_hr=await $self->execute(operation => 'list', %opt);
    die "R2 host returned an invalid list response\n" unless ref($result_hr) eq 'HASH';
    $result_hr->{'objects'}=[map { object($_) } @{(defined($result_hr->{'objects'}) ? $result_hr->{'objects'} : [])}];
    return $result_hr;
}


sub call_host {
    my ($wire)=@_;
    return $HOST_CALL->($wire) if $HOST_CALL;
    no strict 'refs';
    my $host_call_cr=*{'WebDyne::Cloudflare::R2::Host::call'}{'CODE'};
    die "R2 host adapter is not registered in this runtime\n" unless $host_call_cr;
    return $host_call_cr->($wire);
}


async sub execute {
    my ($self, %request)=@_;
    my $wire_hr={
        version    => PROTOCOL_VERSION,
        capability => $self->{'capability'},
        binding    => $self->{'binding'},
        %request,
    };
    my $response_wire=call_host($json_or->encode(WebDyne::Cloudflare::json_value($wire_hr)));
    my $response_hr=eval { $json_or->decode($response_wire) };
    if ((ref($response_hr) ne 'HASH')||!exists($response_hr->{'ok'})) {
        my $detail=$@||'host returned an invalid response';
        die WebDyne::Cloudflare::R2::Error->new(name => 'R2_PROTOCOL_ERROR', message => $detail);
    }
    unless ($response_hr->{'ok'}) {
        my $error_hr=ref($response_hr->{'error'}) eq 'HASH' ? $response_hr->{'error'} : {};
        die WebDyne::Cloudflare::R2::Error->new(%{$error_hr});
    }
    return $response_hr->{'result'};
}

1;
