...
```sh
cp -R examples/hyperdrive /tmp/webdyne-hyperdrive-example
cd /tmp/webdyne-hyperdrive-example
npm install
npm run db:up
npm run check:local
npm run dev:local
...
`db:up` starts the pinned official PostgreSQL image, seeds `schema.sql` on the
...
publishes only `127.0.0.1:15432`.
...
```sh
DB_PORT=15433 npm run db:up
DB_PORT=15433 npm run dev:local
```
